...
$res = $mysqli->query("SELECT s.nomor, s.pertanyaan, s.halaman_ke, j.idjawaban, j.isi_jawaban, j.benarkah FROM soal s JOIN jawaban j ON s.idsoal = j.soal_idsoal WHERE halaman_ke = 1 GROUP BY nomor ");
...
<body>
	<form method="post" action="halaman1_proses.php">	
			...
			<?php  		
				...
				while($row = $res->fetch_assoc())
				{
					...
					$stmt = $mysqli->prepare("SELECT s.nomor, s.pertanyaan, s.halaman_ke, j.idjawaban, j.isi_jawaban, j.benarkah FROM soal s JOIN jawaban j ON s.idsoal = j.soal_idsoal WHERE halaman_ke = 1 AND nomor = ? ");
